...
chmod +x build/lib/arm64-v8a/libvortekrenderer.so

echo "Copying libwinlator.so (v10.1) into build/"
cp prebuilts/libwinlator.so build/lib/arm64-v8a/libwinlator_10_1.so
chmod +x build/lib/arm64-v8a/libwinlator_10_1.so

echo "Copying dump-api layer into build/"
cp prebuilts/libVkLayer_api_dump.so build/lib/arm64-v8a/
chmod +x build/lib/arm64-v8a/libVkLayer_api_dump.so

echo "Adding libraries"
cd build
zip -u base.apk lib/arm64-v8a/libvortekrenderer.so
zip -u base.apk lib/arm64-v8a/libwinlator_10_1.so
